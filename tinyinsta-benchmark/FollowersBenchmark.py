...
class FollowersBenchmark:
    ...
    def run_followers_benchmark(self):
        print(f"\n--- BENCHMARK FOLLOWEES (PARAM={self.param_value}) ---")
        
        self.warmup_specific_prefix()
        
        # Ouvrir en mode append 'a' si le fichier existe déjà, sinon 'w'
        file_exists = os.path.exists('out/fanout.csv')
        mode = 'a' if file_exists else 'w'

        with open('out/fanout.csv', mode, newline='') as csvfile:
        ...
        print("BENCHMARK FOLLOWEES TERMINE pour cette configuration.")

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python3 FollowersBenchmark.py <PARAM_VALUE> <PREFIX>")
        sys.exit(1)
    ...
    benchmark = FollowersBenchmark(BASE_URL, param_value, prefix)
